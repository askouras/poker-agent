...
money = 100
# Amount of money for ante
anteing = 10
# Amount of money for raise
raising = 10
...
def ante():
    ...
    global pot
    players_money = [player_money - anteing for player_money in players_money]
    for player_money in players_money:
        pot += anteing
        player_money -= anteing
...
def raised(player_index):
    ...
    if players_money[player_index] >= raising:
        pot += raising
        players_money[player_index] -= raising
...
def startOpponent(index):
    ...
    if (opponentStrategy == 1):
        ...
        # Raise if player has at least half of their money left, and better 
         #   than a straight
        if (players_money[index]  > (money / 2)) or (rank < 6):
            raised(index)
        else:
            stay(index)
